...
import config
import GraphGAN.generator as Generator
import GraphGAN.critic as Critic
from src import utils
from src.evaluation import link_prediction
...
class GraphGAN(object):
    def __init__(self):
        ...
        print(f"Number of Nodes: {self.n_node}")
    ...
    def train(self):
        ...
        for epoch in range(config.n_epochs):
            ...
            for g_epoch in range(config.n_epochs_gen):
                ...
                neighbor_nodes = []
                labels = []
                for d_epoch in range(config.n_epochs_critic):
                    # generate new nodes for the critic for every dis_interval iterations
    ...
    def write_embeddings_to_file(self):
        ...
        for i in range(2):
            ...
            with open(config.emb_filenames[i], "w+") as f:
                lines = [str(self.n_node) + "\t" + str(config.n_embed) + "\n"] + embedding_str
                f.writelines(lines)
    ...
    def evaluation(self):
            ...
            if config.app == "link_prediction":
                for i in range(2):
                    lpe = link_prediction(
                        config.emb_filenames[i], config.test_filename, config.test_neg_filename, self.n_node, config.n_embed)
                    result , macro_f1 = lpe.eval_link_prediction()
                    results.append(config.modes[i] + ":" + str(result) + "\n")
                    results.append(config.modes[i] + "_macrof1" + ":" + str(macro_f1) + "\n")
