...
def MetricDictionary(files):
    metrics = {'nonfuzzy': {'gen': [],
                            'gen_macrof1': [],
                            'dis': [],
                            'dis_macrof1': []},
                'fuzzy': {'gen': [],
    ...
    for key in files:
        with open(files[str(key)], 'r') as file:
            lines = file.readlines()
            for line in lines:
                if line[:4] == 'gen:':
                    accuracy_values = float(line[4:].strip())
                    metrics[str(key)][line[:3]].append(accuracy_values)
                    ...
                    accuracy_values = float(line[12:].strip())
                    metrics[str(key)][line[:11]].append(accuracy_values)
                else:
                    accuracy_values = float(line[12:].strip())
                    metrics[str(key)][line[:11]].append(accuracy_values)

    for key1 in metrics:
        for key2 in metrics[str(key)]:
            metrics[str(key1)][str(key2)] = np.array(metrics[str(key1)][str(key2)])
