...
class RandomWalker:
    ...
    def node2vec_walk(self, walk_length, start_node):
        ...
        while len(walk) < walk_length:
            ...
            if len(cur_nbrs) > 0:
                if len(walk) == 1:
                    walk.append(
                        cur_nbrs[utils.alias_sample(alias_nodes[cur][0], alias_nodes[cur][1])])
                else:
                    prev = walk[-2]
                    edge = (prev, cur)
                    next_node = cur_nbrs[utils.alias_sample(alias_edges[edge][0],
                                                      alias_edges[edge][1])]
    ...
    def preprocess_transition_probs(self):
        ...
        for node in G.nodes():
            ...
            alias_nodes[node] = utils.create_alias_table(normalized_probs)
    ...
    def get_alias_edge(self, t, v):
        ...
        return utils.create_alias_table(normalized_probs)
...
def chooseNeighbor(v, graphs, layers_alias, layers_accept, layer):
    ...
    idx = utils.alias_sample(layers_accept[layer][v], layers_alias[layer][v])
    v = v_list[idx]
