    ...
    def _compute_structural_distance(self, max_num_layers, workers=1, verbose=0, ):
        ...
        if os.path.exists(self.temp_path + 'structural_dist.pkl'):
            ...
            results = Parallel(n_jobs=workers, verbose=verbose, )(
                ...
                utils.partition_dict(vertices, workers))
    ...
    def _get_transition_probs(self, layers_adj, layers_distances):
        ...
        for layer in layers_adj:
            ...
            for v, neighbors in neighbors.items():
                ...
                accept, alias = utils.create_alias_table(e_list)
...
struc2vec = Struc2Vec(graph=G, walk_length=config.walk_length, num_walks= config.num_walks, workers=4, verbose=40, ) #init model
model = Word2Vec(struc2vec.sentences, epochs = config.epochs, vector_size=config.vector_size, window=config.window_size)
